...
class _SettingsScreenState extends State<SettingsScreen> {
    ...
    if (svc != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('✅ 웹훅 URL이 저장되었습니다'), backgroundColor: Colors.green),
        );
      }
    }
  }
  ...
  Future<void> _testConnection() async {
    ...
    try {
      await svc.saveWebhookUrl(_urlController.text.trim());
      final dummyContact = TMContact(
        id: 'test_001', name: '테스트 연결', phone: '010-0000-0000',
        resultCodes: ['통화가능'], customerGrade: 'A', memo: '앱 연결 테스트',
        callDuration: 30, retryCount: 0, isCompleted: true, isSkipped: false,
      );
      await svc.syncResult(sessionId: 'test_session', sessionName: '연결 테스트', contact: dummyContact);
      setState(() { _isTesting = false; _testResult = '✅ 연결 성공! 구글 시트에 테스트 데이터가 기록되었습니다.'; _testSuccess = true; });
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Card(child: const ListTile(leading: Icon(Icons.info_outline), title: Text('TM Auto Caller'), subtitle: Text('v4.0.3 · 개발: TM Auto Caller'))),
        ],
      ),
    );
  }
