...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _testConnection() async {
    ...
    if (svc == null) {
      setState(() {
        _isTesting = false;
        _testResult = 'Service init error';
        _testSuccess = false;
      });
      return;
    }
    try {
      await svc.saveWebhookUrl(_urlController.text.trim());
      final testContact = TMContact(
        id: 1,
        name: 'Test User',
        phone: '010-0000-0000',
        resultCodes: const ['callable'],
        customerGrade: 'A',
      ...
      setState(() {
        _isTesting = false;
        _testResult = 'Connected! Check Google Sheet.';
        _testSuccess = true;
      });
    } catch (e) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const _SectionTitle('Google Sheet Sync'),
          Card(
            child: Padding(
              padding: const EdgeInsets.all(16),
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  const Text(
                    'Auto-sync call results to Google Sheet',
